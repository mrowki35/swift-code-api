...
banks_by_country_router.get(
    ...
    async (req: Request, res: Response): Promise<void> => {
      ...
      try {
        ...
        const hedquarters = await executeQuery(conn,  banks_by_country_query , [countryISO2code]); 
        const countryName = await executeQuery(conn,  country_name_query , [countryISO2code]); 
